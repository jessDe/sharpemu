...
internal static class GpuWaitRegistry
{
    public struct WaitingDcb
    {
        ...
        public bool Latched;
        // Set when the submitted command stream contains a producer for this
        // label (or when a producer has already written it). Producerless AGC
        // compute waits can otherwise suspend forever when the packet that
        // would have generated the label update is not yet emulated.
        public bool ProducerObserved;
        // True when ProducerObserved came only from a previous write to this
        // reused label. A producer queued for the current submission clears it.
        public bool ProducerObservedFromHistory;
    ...
    private static readonly bool _traceOrphanSkips =
        Environment.GetEnvironmentVariable("SHARPEMU_TRACE_ORPHAN_SKIPS") != "0";
    private static readonly HashSet<ulong> _reportedOrphanSkips = [];
    ...
    public static void Register(ulong address, WaitingDcb waiter)
    {
        ...
        lock (_gate)
        {
            if (waiter.Memory is not null && _lastProduced.ContainsKey((waiter.Memory, address)))
            {
                waiter.ProducerObserved = true;
                waiter.ProducerObservedFromHistory = true;
            }
    ...
    /// <summary>
    /// Marks existing waiters whose watched label overlaps a producer packet.
    /// This is separate from satisfaction: a queued producer must keep normal
    /// ordering even before its ordered write has executed.
    /// </summary>
    public static void MarkProducerObservedInRange(
        object memory,
        ulong start,
        ulong length)
    {
        ...
        lock (_gate)
        {
            foreach (var (address, list) in _waiters)
            {
                for (var index = 0; index < list.Count; index++)
                {
                    ...
                    if ((waiter.ProducerObserved && !waiter.ProducerObservedFromHistory) ||
                        !ReferenceEquals(waiter.Memory, memory))
                    {
                        continue;
                    }

                    var width = waiter.Is64Bit ? (ulong)sizeof(ulong) : sizeof(uint);
                    ...
                    waiter.ProducerObserved = true;
                    waiter.ProducerObservedFromHistory = false;
                    list[index] = waiter;
                }
            }
        }
    ...
    /// <summary>
    /// Moves one indirect-dispatch retry deadline to <paramref name="nowTicks"/>
    /// after an ordered visibility point proved that its producing queue has
    /// completed without generating valid dimensions.
    /// </summary>
    public static bool ExpireRetry(
        object memory,
        ulong resumeAddress,
        long nowTicks)
    {
        lock (_gate)
        {
            foreach (var (_, list) in _waiters)
            {
                for (var index = 0; index < list.Count; index++)
                {
                    ...
                    waiter.RetryDeadlineTicks = nowTicks;
                    list[index] = waiter;
                    return true;
                }
            }
    ...
    /// <summary>
    /// Removes old producerless equality waits from AGC compute queues. These
    /// are compatibility waits generated by sceAgc's NOP packet form; without
    /// an observed producer they cannot ever complete. Standard PM4 waits and
    /// waits with a producer queued for the current submission always fail
    /// closed. A producer remembered from an older use of the label only
    /// extends the grace period.
    /// </summary>
    public static List<WaitingDcb>? CollectExpiredOrphanedComputeWaits(
        object memory,
        long nowTicks,
        long minAgeTicks)
    {
        List<WaitingDcb>? orphaned = null;
        lock (_gate)
        {
            List<ulong>? emptied = null;
            foreach (var (address, list) in _waiters)
            {
                ...
                {
                    var waiter = list[index];
                    var historicalProducer =
                        waiter.ProducerObserved && waiter.ProducerObservedFromHistory;
                    var requiredAge = historicalProducer
                        ? minAgeTicks * 64
                        : minAgeTicks;
                    if (!ReferenceEquals(waiter.Memory, memory) ||
                        (waiter.ProducerObserved && !historicalProducer) ||
                        waiter.RetryDeadlineTicks != 0 ||
                        waiter.IsStandard ||
                        waiter.CompareFunction != 3 ||
                        waiter.Mask == 0 ||
                        string.IsNullOrEmpty(waiter.QueueName) ||
                        !waiter.QueueName.StartsWith("acb.compute[", StringComparison.Ordinal) ||
                        nowTicks - waiter.RegisteredTicks < requiredAge)
                    {
                        if (_traceOrphanSkips &&
                            nowTicks - waiter.RegisteredTicks >= minAgeTicks * 4 &&
                            _reportedOrphanSkips.Add(waiter.WaitAddress))
                        {
                            var reason = !ReferenceEquals(waiter.Memory, memory)
                                ? "other-memory"
                                : waiter.ProducerObserved
                                    ? historicalProducer
                                        ? "historical-producer-grace"
                                        : "producer-observed"
                                    : waiter.RetryDeadlineTicks != 0
                                        ? "retry-deadline-set"
                                        : waiter.IsStandard
                                            ? "standard-pm4"
                                            : waiter.CompareFunction != 3
                                                ? $"cmp={waiter.CompareFunction}"
                                                : waiter.Mask == 0
                                                    ? "mask=0"
                                                    : string.IsNullOrEmpty(waiter.QueueName)
                                                        ? "no-queue-name"
                                                        : !waiter.QueueName.StartsWith(
                                                            "acb.compute[",
                                                            StringComparison.Ordinal)
                                                            ? $"queue={waiter.QueueName}"
                                                            : "too-young";
                            Console.Error.WriteLine(
                                $"[LOADER][WARN] agc.orphan_skip " +
